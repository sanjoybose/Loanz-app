...
var validator = (function($){
	...
	*/
	tests = {
		...
		hasValue : function(a, field){
			if( !a ){
			    alertTxt = (field && field.data('message-required')) ? field.data('message-required') : message.empty;
				return false;
			}
			return true;
		},
	...
	*/
    mark = function( field, text ){
		...
		var item = field.parents('.formItem'),
			warning;
		...
		if( item.hasClass('bad') ){
            ...
            if (field.data('type') == 'checkbox') {
                warning.css({ 'left': $(item).outerWidth() + "px" });
            }
            else {
                warning.css({ 'left': ($(field).outerWidth() + 5) + "px" });
            }
            item.append( warning );
        }
	...
	*/
	...
	function prepareFieldData(el){
		...
		pattern = field.data('validate-pattern');
	}
	...
	*/
	function checkField_OLD(){
		...
		if (this.type == 'checkbox') {
		    prepareFieldData(this);
		    data.valid = $(this).is(':checked');

		    // Check if there is a specific error message for that field, if not, use the default 'invalid' message
		    alertTxt = field.data('message-required') || message[field.prop('name')] || message.invalid;
    ...
    /* Checks a single form field by it's type and specific (custom) attributes with a overload to skip
	*/
