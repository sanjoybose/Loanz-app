...
(function($, undefined) {
    ...
    var SliderBase = Widget.extend({
        init: function(element, options) {
            ...
            options.tickLabels = that.options.tickLabels ? that.options.tickLabels : [];
        ...
        options: {
            ...
            tickPlacement: "both",
            tickLabels: [],
            tooltip: { enabled: true, format: "{0}", autoHide: true }
        },
        ...
        _setItemsTitle: function() {
            var that = this,
                options = that.options,
                itemLabels = that.options.tickLabels, //itemLabels = ["Poor", "Bad", "Good", "Excellent"],
                items = that.wrapper.find(TICK_SELECTOR),
            ...
            if (itemLabels.length == 0) {
                ...
                for (; i - limit !== 0 ; i += increment) {
                    $(items[i]).attr("title", format(options.tooltip.format, itemLabels[i]));
                    titleNumber += options.smallStep;
                }
            }
        },
            ...
            if ((1000 * options.largeStep) % (1000 * options.smallStep) === 0 || that._distance / options.largeStep >= 3) {
                ...
                for (i = 0; i < items.length; i++) {
                    ...
                    if (value % options.smallStep === 0 && value % options.largeStep === 0) {
                        if (that.options.showTickText == false) {
                            item.addClass("k-tick-large");
                        }
                        else {
                            if (that.options.tickFormat == "K") {
                                var t = item.attr("title").replace(/[^0-9.]/g, '');
                                t = format(options.tooltip.format, t / 1000) + "K";
                                item.addClass("k-tick-large")
                                    .html("<span class='k-label'>" + t + "</span>");
                            }
                            else {
                                item.addClass("k-tick-large")
    ...
    Slider.Drag.prototype = {
        dragstart: function (e) {
            ...
            this._updateTooltip(this.owner.value());
        },
        ...
        _createTooltip: function() {
            ...
            if (!that.options.tooltip.autoHide)
            {
                that.tooltipDiv.click(function () {
